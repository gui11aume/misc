...
if __name__ == "__main__":
   ...
   with torch.cuda.device(3):
      ...
      if len(sys.argv) > 1:
         encd.load_state_dict(torch.load('aae-encd-1124.tch'))
         decd.load_state_dict(torch.load('aae-decd-1124.tch'))
      ...
      asched = torch.optim.lr_scheduler.MultiStepLR(aopt, [800, 1000])
      bsched = torch.optim.lr_scheduler.MultiStepLR(bopt, [800, 1000])
      csched = torch.optim.lr_scheduler.MultiStepLR(copt, [800, 1000])
      ...
      for epoch in range(1124):
         ...
         for batch in train.batches(btchsz=128):
            ...
            cstr = decd(Z0)
            cstr_loss = loss_cstr(cstr, batch) * 10000
      ...
      torch.save(encd.state_dict(), 'aae-encd-%d.tch' % (epoch+1))
      torch.save(decd.state_dict(), 'aae-decd-%d.tch' % (epoch+1))
      torch.save(disc.state_dict(), 'aae-disc-%d.tch' % (epoch+1))
