...
if __name__ == "__main__":
   ...
   #   'optimizer': optim,
   #   'optim_args': {'lr': 0.001, 'betas': (0.90, 0.999)},
   ...
   for batch in data.batches(btchsz=256):
      ...
      z = Normal(infmu, infsd).sample()
      # Propagate forward and sample observable 'x'.
      with torch.no_grad():
         mu,sd = genr.detfwd(z)
      x = Normal(mu[:,:45],sd[:,:45]).sample()
