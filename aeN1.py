...
if __name__ == "__main__":
   ...
   if len(sys.argv) > 1:
      encd.load_state_dict(torch.load('aeN1-encd-1000.tch'))
      decd.load_state_dict(torch.load('aeN1-decd-1000.tch'))
   ...
   for epoch in range(1000):
      ...
      for batch in train.batches(btchsz=128):
         ...
         noise = torch.randn(batch.shape) * .05
         loss = compute_batch_loss(batch + noise, device)
   ...
   torch.save(encd.state_dict(), 'aeN1-encd-%d.tch' % (epoch+1))
   torch.save(decd.state_dict(), 'aeN1-decd-%d.tch' % (epoch+1))
