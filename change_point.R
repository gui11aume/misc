#r <- function(d, x, t) {
#   return(x^d * exp(-x^2/2) / (2^(d/2) * gamma(d/2)) * (t + (4-d*t)/x^2))
#}
...
#
#   obs_2[i] = sqrt(max(2*l1 +2*l2 - 2*l0))
#}

source('mlNB.R')

#x <- rnbinom(1000, size=3.4, prob=0.2)
x <- rnbinom(1000, size=0.9, prob=rep(c(0.3,0.5,0.3,0.5), each=250))
tab <- tabulate(x+1L)
u <- 0:(length(tab)-1L)
n <- length(x)
ap <- mlNB(x)
...
plot(sqrt(lambda), type='l')
print(r(max(sqrt(lambda), na.rm=TRUE), n))
