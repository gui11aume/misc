...
class Classifier(nn.Module):
   def __init__(self):
      ...
      self.embed_context = nn.Linear(45, self.d_model)
...
if __name__ == "__main__":
   ...
   wgt = torch.tensor([10.,1.]).to(device)
   loss_fun = nn.CrossEntropyLoss(weight=wgt, reduction='mean')
   for epoch in range(200):
      ...
      for neg, crv in train.batches(btchsz=92):
         ...
         ctx1 = ctx[:,:ctx.shape[1]//2,:].to(device)
         cls1 = cls[:,:cls.shape[1]//2,:].to(device)
         neg1 = neg[:cls.shape[1]//2,:].to(device)

         ctx2 = ctx[:,ctx.shape[1]//2:,:].to(device)
         cls2 = cls[:,cls.shape[1]//2:,:].to(device)
         neg2 = neg[cls.shape[1]//2:,:].to(device)

         # Flip...
         loss = loss_fun(model(cls1, ctx1).squeeze(), neg1.squeeze()) / 2
         # ... flop!
         loss += loss_fun(model(cls2, ctx2).squeeze(), neg2.squeeze()) / 2
      ...
      sys.stderr.write('Epoch %d, train: %f, test: %f\n' % \
         (epoch+1, epoch_loss / n_train_batches, test_loss / n_test_batches))

   # Done, save the networks.
   torch.save(model.state_dict(), 'crv-disc-%d.tch' % (epoch+1))
